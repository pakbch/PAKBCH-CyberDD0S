...
func main() {
	...
	fmt.Println("Select a mode (type in the number): " + color.Red.Render("[1] NUKE ") + color.Yellow.Render("[2] NORMAL ") + color.Green.Render("[3] SLOW"))
...
func getList() []string {
	for {
		req := fasthttp.AcquireRequest()
		req.SetRequestURI("put your pastebin link here") //example: https://pastebin.com/raw/sV2FnN2r
		res := fasthttp.AcquireResponse()
